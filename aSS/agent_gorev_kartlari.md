...
> **Proje:** `c:\RMSv3` — LAN Master-Slave POS Desktop Dönüşümü  
...
**1. `c:\RMSv3\desktop\terminalConfig.cjs` (YENİ)**
...
**2. `c:\RMSv3\src\lib\terminalIdentity.js` (YENİ)**
...
- `c:\RMSv3\src\lib\terminalIdentity.js` (FAZ 0-A çıktısı)
- `c:\RMSv3\.antigravityrules.md` (stil kuralları)
- `c:\RMSv3\src\index.css` (mevcut CSS değişkenleri)

### Oluşturulacak Dosya
`c:\RMSv3\src\components\pos\PairingScreen.jsx` (YENİ)
...
### Oluşturulacak Dosya
`c:\RMSv3\migrations\020_pos_terminal_registry.sql` (YENİ)
...
### Oluşturulacak Dosya
`c:\RMSv3\desktop\sqliteStore.cjs` (YENİ)
...
### Bağlam Dosyaları — Önce Oku
- `c:\RMSv3\desktop\sqliteStore.cjs` (FAZ 1 çıktısı)
- `c:\RMSv3\desktop\terminalConfig.cjs` (FAZ 0-A çıktısı)
- `c:\RMSv3\server\index.js` — Referans: mevcut API server'ın `buildConditions` ve `executeQuery` mantığını anla

### Oluşturulacak Dosya
`c:\RMSv3\desktop\edgeServer.cjs` (YENİ)
...
### Oluşturulacak Dosya
`c:\RMSv3\desktop\syncWorker.cjs` (YENİ)
...
### Önce Oku
`c:\RMSv3\src\lib\db.js` — TAMAMEN oku. Mevcut `QueryBuilder` sınıfı ve `queryApi` fonksiyonunu anla.
...
### Dosya 1: `c:\RMSv3\src\hooks\useLanSync.js` (YENİ)
...
### Dosya 2: `c:\RMSv3\src\components\pos\OfflineStatusBar.jsx` (YENİ)
...
### Önce Oku
- `c:\RMSv3\desktop\main.cjs` — TAMAMEN oku
- `c:\RMSv3\src\DesktopPosApp.jsx` — TAMAMEN oku
- `c:\RMSv3\desktop\terminalConfig.cjs` (FAZ 0-A)
- `c:\RMSv3\desktop\edgeServer.cjs` (FAZ 2)
- `c:\RMSv3\desktop\syncWorker.cjs` (FAZ 3)
...
### Değişiklik Yeri
`c:\RMSv3\src\lib\terminalIdentity.js` (FAZ 0-A çıktısı) — fonksiyon ekle.
...
### db.js'e Entegrasyon
`c:\RMSv3\src\lib\db.js` içinde INSERT ve UPSERT işlemlerinin `data` parametresi build edildiği yerde:
...
### Dosya 1: `c:\RMSv3\desktop\updater.cjs` (YENİ)
