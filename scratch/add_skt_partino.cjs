...
async function run() {
  ...
  await client.query(`
    ALTER TABLE public.quality_reports ADD COLUMN IF NOT EXISTS skt DATE;
    ALTER TABLE public.quality_reports ADD COLUMN IF NOT EXISTS parti_no TEXT;
  `);
  console.log('Columns skt and parti_no added successfully to quality_reports table.');
  await client.end();
}
