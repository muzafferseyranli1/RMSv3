async function run() {
  ...
  try {
    ...
    // Test 4: p_sales_channel as null (no channel filter)
    console.log('--- TEST 4: Sending p_sales_channel as null ---');
    try {
    ...
    // Test 5: p_sales_channel as 'Hızlı Satış'
    console.log('--- TEST 5: Sending p_sales_channel as Hızlı Satış ---');
    try {
      const rpcRes = await fetch('https://rms-api-production-219d.up.railway.app/api/query', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
