...
async function getDatabaseUrl() {
  ...
  try {
    ...
    // ignore, fall back to checking if pg env is set
  }
  return null
}

const migrationPath = path.resolve(__dirname, '..', 'migrations', '012_loyalty_period_product_quantity_filters.sql')
...
main().catch(error => {
  console.error('Migration 012 failed:', error?.message || error)
  process.exit(1)
})
