...
async function run() {
  ...
  try {
    ...
    const branches = allNodes.filter(n => n.type === 'sube').map(b => {
      let company_id = null
    ...
    for (const branch of branches) {
      ...
      for (const targetDate of TARGET_DATES) {
        ...
        if (currentCount >= 160) {
          console.log(`[${targetDate}] Atlanıyor. Yeterli satış var (${currentCount} adet).`)
          continue
        }

        if (currentCount > 0 && currentCount < 160) {
          console.log(`[${targetDate}] Test satışları bulundu (${currentCount} adet). Temizleniyor...`)
