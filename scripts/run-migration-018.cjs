...
async function run() {
  ...
  const sql = fs.readFileSync(path.resolve(__dirname, '..', 'migrations', '018_inventory_cost_calculation_fix.sql'), 'utf8');
  await client.query(sql);
  console.log('Migration 018 applied successfully to Railway Postgres');
  await client.end();
}
