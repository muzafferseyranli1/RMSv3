...
async function run() {
  ...
  const sql = fs.readFileSync(path.resolve(__dirname, '..', 'migrations', '019_task_form_template_relation.sql'), 'utf8');
  await client.query(sql);
  console.log('Migration 019 applied successfully to Railway Postgres');
  await client.end();
}
