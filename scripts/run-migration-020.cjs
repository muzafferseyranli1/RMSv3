...
async function run() {
  ...
  const sql = fs.readFileSync(path.resolve(__dirname, '..', 'migrations', '020_pos_terminal_registry.sql'), 'utf8');
  await client.query(sql);
  console.log('Migration 020 applied successfully to Railway Postgres');
  await client.end();
}
