...
export default function EInvoiceManager() {
  ...
  // State
  const [activeTab, setActiveTab] = useState('inbox') // 'inbox', 'outbox', 'intercompany', 'eadisyon', 'simulator', 'settings'
  ...
  // Simulator Selected Invoice for Status Transition
  const [simSelectedInvoiceId, setSimSelectedInvoiceId] = useState('')
  const [simTargetStatus, setSimTargetStatus] = useState(1300)
  ...
  const handleRunScenario = async (scenarioKey) => {
    setGeneratingScenario(scenarioKey)
    try {
      let created = null
      if (scenarioKey === 'INTER_COMPANY') {
        const res = await interCompanyTransferService.simulateInterCompanyTransferInvoice()
        if (res.success) {
          toast(`🏢 Şirketler arası transfer faturası (${res.invoiceNumber}) başarıyla oluşturuldu ve eşleştirildi.`, 'success')
          await loadData()
          return
        } else {
          throw new Error(res.error || 'Şirketler arası fatura simülasyonu başarısız.')
        }
      } else if (scenarioKey === 'OUTBOUND_SIM') {
        created = await eInvoiceService.generateTestOutboundInvoice()
      } else {
        created = await eInvoiceService.generateTestInvoice(scenarioKey)
      }

      if (created) {
        toast(`Simülasyon faturası (${created.invoice_number}) başarıyla oluşturuldu.`, 'success')
        await loadData()
      } else {
        toast('Simülasyon faturası oluşturulamadı.', 'error')
      }
    } catch (err) {
      toast(err.message, 'error')
    } finally {
  ...
  const handleTriggerStatusChange = async () => {
    if (!simSelectedInvoiceId) {
      toast('Lütfen bir fatura seçin.', 'error')
      return
    }

    try {
      const res = await eInvoiceService.updateInvoiceStatus(simSelectedInvoiceId, Number(simTargetStatus))
      if (res.success) {
        toast(`Fatura durumu ${simTargetStatus} olarak güncellendi.`, 'success')
        await loadData()
      } else {
  ...
  return (
    <div style={{ maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button
            ...
            disabled={Boolean(generatingScenario)}
            style={{
              display: 'flex',
              alignItems: 'center',
              gap: 6,
          ...
          >
            ...
            {generatingScenario === 'INTER_COMPANY' ? 'Simüle Ediliyor...' : 'Şirketler Arası Fatura Simüle Et'}
          </button>
      ...
      <div style={{ display: 'flex', gap: 8, borderBottom: '1px solid var(--border)', marginBottom: 20, overflowX: 'auto' }}>
        ...
        <button
          ...
          style={{
            ...
            border: 'none',
            background: 'transparent',
            color: activeTab === 'simulator' ? '#f5a623' : 'var(--text-muted)',
            borderBottom: activeTab === 'simulator' ? '2px solid #f5a623' : '2px solid transparent',
        ...
        >
          <i className="fa-solid fa-flask-vial" />
          Entegratör Simülatörü & Senaryolar
        </button>
      ...
      {/* Tab 3: Simulator & Scenarios Tab */}
      {activeTab === 'simulator' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: 20 }}>
          {/* Sol Kolon: Hazır Test Senaryoları */}
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 20 }}>
            <h2 style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--text-strong)', margin: '0 0 8px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
              <i className="fa-solid fa-bolt" style={{ color: '#f5a623' }} />
              1-Tıkla Simülatör Senaryo Başlatıcı
            </h2>
            <p style={{ fontSize: '.85rem', color: 'var(--text-muted)', margin: '0 0 18px 0' }}>
              Gerçek Türk restoran tedarikçileri (Et, Süt, İçecek, Unlu Mamuller, Sebze/Meyve, Ambalaj) ile test e-faturaları oluşturun:
            </p>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {/* Scenario 1 */}
              <div style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 14, background: 'var(--app-bg)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontWeight: 800, color: '#10b981', fontSize: '.9rem', display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    <i className="fa-solid fa-circle-check" />
                    Tam Eşleşen Fatura
                  </div>
                  <div style={{ fontSize: '.8rem', color: 'var(--text-muted)', lineHeight: 1.4, marginBottom: 12 }}>
                    Sipariş ve irsaliye teslimatı ile %100 birebir tutan, fiyat veya miktar farkı olmayan standart tedarikçi faturası.
                  </div>
                </div>
                <button
                  type="button"
                ...
                >
                  {generatingScenario === 'MATCHED' ? 'Oluşturuluyor...' : 'Oluştur'}
                </button>
              </div>
              ...
              <div style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 14, background: 'var(--app-bg)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontWeight: 800, color: '#f59e0b', fontSize: '.9rem', display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    <i className="fa-solid fa-arrow-trend-up" />
                    Fiyat Farklı Fatura (+%25)
                  </div>
                  <div style={{ fontSize: '.8rem', color: 'var(--text-muted)', lineHeight: 1.4, marginBottom: 12 }}>
                    Sözleşme/sipariş birim fiyatından %25 daha yüksek kesilmiş kalem içerir (Fiyat sapması uyarısı simülasyonu).
                  </div>
                </div>
                <button
                  type="button"
                  disabled={Boolean(generatingScenario)}
                  onClick={() => handleRunScenario('PRICE_DIFF')}
                  style={{
                    padding: '8px 12px',
                    borderRadius: 8,
                    border: 'none',
                ...
                >
                  {generatingScenario === 'PRICE_DIFF' ? 'Oluşturuluyor...' : 'Oluştur'}
                </button>
              </div>
              ...
              <div style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 14, background: 'var(--app-bg)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontWeight: 800, color: '#ef4444', fontSize: '.9rem', display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    <i className="fa-solid fa-boxes-packing" />
                    Eksik Teslimat Faturası
                  </div>
                  <div style={{ fontSize: '.8rem', color: 'var(--text-muted)', lineHeight: 1.4, marginBottom: 12 }}>
                    İrsaliyede teslim alınandan +10 adet fazla faturalandırılmış (Mal kabul miktar uyuşmazlığı simülasyonu).
                  </div>
                </div>
                <button
                  type="button"
                  disabled={Boolean(generatingScenario)}
                  onClick={() => handleRunScenario('SHORTAGE')}
                  style={{
                    padding: '8px 12px',
                    borderRadius: 8,
                    border: 'none',
                ...
                >
                  {generatingScenario === 'SHORTAGE' ? 'Oluşturuluyor...' : 'Oluştur'}
                </button>
              </div>
              ...
              <div style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 14, background: 'var(--app-bg)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontWeight: 800, color: '#a78bfa', fontSize: '.9rem', display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    <i className="fa-solid fa-building" />
                    Yeni / Tanımsız Tedarikçi
                  </div>
                  <div style={{ fontSize: '.8rem', color: 'var(--text-muted)', lineHeight: 1.4, marginBottom: 12 }}>
                    Sistemde henüz cari hesabı veya stok eşleştirmesi olmayan yeni bir tedarikçi faturası simülasyonu.
                  </div>
                </div>
                <button
                  type="button"
                  disabled={Boolean(generatingScenario)}
                  onClick={() => handleRunScenario('NEW_SUPPLIER')}
                  style={{
                    padding: '8px 12px',
                    borderRadius: 8,
                    border: 'none',
                ...
                >
                  {generatingScenario === 'NEW_SUPPLIER' ? 'Oluşturuluyor...' : 'Oluştur'}
                </button>
              </div>
              ...
              <div style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 14, background: 'var(--app-bg)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontWeight: 800, color: '#22d3ee', fontSize: '.9rem', display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    <i className="fa-solid fa-file-invoice" />
                    e-Arşiv Fatura (B2C)
                  </div>
                  <div style={{ fontSize: '.8rem', color: 'var(--text-muted)', lineHeight: 1.4, marginBottom: 12 }}>
                    Vergi mükellefi olmayan nihai tüketiciye düzenlenen e-Arşiv fatura formatı simülasyonu.
                  </div>
                </div>
                <button
                  type="button"
                  disabled={Boolean(generatingScenario)}
                  onClick={() => handleRunScenario('EARSIV')}
                  style={{
                    padding: '8px 12px',
                    borderRadius: 8,
                    border: 'none',
                ...
                >
                  {generatingScenario === 'EARSIV' ? 'Oluşturuluyor...' : 'Oluştur'}
                </button>
              </div>
              ...
              <div style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 14, background: 'var(--app-bg)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontWeight: 800, color: '#f5a623', fontSize: '.9rem', display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    <i className="fa-solid fa-paper-plane" />
                    Giden Satış Faturası
                  </div>
                  <div style={{ fontSize: '.8rem', color: 'var(--text-muted)', lineHeight: 1.4, marginBottom: 12 }}>
                    Restoranımızın kurumsal müşteriye kestiği catering / organizasyon e-faturası simülasyonu.
                  </div>
                </div>
                <button
                  type="button"
                ...
                >
                  {generatingScenario === 'OUTBOUND_SIM' ? 'Oluşturuluyor...' : 'Oluştur'}
                </button>
              </div>

              {/* Scenario 7: Inter-Company Transfer Invoice */}
              <div style={{ gridColumn: 'span 2', border: '1px solid #fed7aa', borderRadius: 10, padding: 14, background: 'linear-gradient(135deg, #fffbeb 0%, #fff7ed 100%)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: 10 }}>
                <div>
                  <div style={{ fontWeight: 800, color: '#c2410c', fontSize: '.95rem', display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    <i className="fa-solid fa-building-circle-arrow-right" />
                    🏢 Şirketler Arası Transfer Faturası (Inter-Company Transfer Invoicing)
                    <span className="badge" style={{ background: '#ea580c', color: '#fff', fontSize: '.68rem', padding: '2px 8px', borderRadius: 6 }}>
                      FAZ 3
                    </span>
                  </div>
                  <div style={{ fontSize: '.82rem', color: '#9a3412', lineHeight: 1.5 }}>
                    Farklı vergi kimlik numaralarına (VKN) sahip tüzel kişilikler / şubeler arası ürün sevkiyatlarında, standart UBL-TR 2.1 E-Fatura ve E-İrsaliye referanslı belge oluşturur. Gönderici adına GİDEN kutusuna, alıcı tüzel kişilik adına GELEN kutusuna 3-way eşleşme için anında ayna faturayı işler.
                  </div>
                </div>
                <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                  ...
                  >
                    <i className="fa-solid fa-building-circle-arrow-right" />
                    {generatingScenario === 'INTER_COMPANY' ? 'Şirketler Arası Fatura Oluşturuluyor...' : 'Şirketler Arası Transfer Faturası Simüle Et'}
                  </button>
                </div>
              </div>
            </div>
          ...
          {/* Sağ Kolon: Durum Yaşam Döngüsü & Geçiş Simülatörü */}
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 20 }}>
            <h2 style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--text-strong)', margin: '0 0 8px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
              <i className="fa-solid fa-timeline" style={{ color: '#f5a623' }} />
              Fatura Durumu Yaşam Döngüsü
            </h2>
            <p style={{ fontSize: '.85rem', color: 'var(--text-muted)', margin: '0 0 16px 0' }}>
              Herhangi bir faturayı seçerek GİB/Entegratör durum kodunu manuel veya adım adım ilerletin:
            </p>

            <div style={{ marginBottom: 14 }}>
              <label style={{ display: 'block', fontSize: '.8rem', fontWeight: 600, color: 'var(--text-muted)', marginBottom: 6 }}>Hedef Faturayı Seçin:</label>
              <select
              ...
              >
                {invoices.map((inv) => (
                  <option key={inv.id} value={inv.id}>
                    {inv.invoice_number} - {inv.sender_title || inv.receiver_title} ({inv.status_description})
                  </option>
                ))}
              </select>
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', fontSize: '.8rem', fontWeight: 600, color: 'var(--text-muted)', marginBottom: 6 }}>Yeni Durum Kodu:</label>
              <select
                value={simTargetStatus}
                onChange={(e) => setSimTargetStatus(Number(e.target.value))}
                style={{
                  width: '100%',
                  padding: '10px 12px',
                  borderRadius: 8,
              ...
              >
                <option value="1000">1000 - Taslak / Kuyrukta</option>
                <option value="1100">1100 - Entegratöre Gönderildi</option>
                <option value="1120">1120 - GİB'e İletildi</option>
                <option value="1163">1163 - GİB'de İşlendi (Zarf Başarılı)</option>
                <option value="1200">1200 - Alıcıya Ulaştı (Kabul/Red Bekliyor)</option>
                <option value="1300">1300 - Kabul Edildi (Onaylandı)</option>
                <option value="1301">1301 - Reddedildi</option>
                <option value="9999">9999 - Hatalı / Başarısız</option>
              </select>
            ...
            >
              ...
              Durumu Güncelle & Uygula
            </button>

            {/* Lifecycle Stages Preview */}
            <div style={{ borderTop: '1px solid var(--border)', paddingTop: 16 }}>
              <div style={{ fontSize: '.75rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: 10 }}>GİB E-Fatura Aşamaları</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: '.8rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#888' }}>
                  <span style={{ width: 20, height: 20, borderRadius: '50%', background: 'var(--surface-2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.7rem', fontWeight: 700 }}>1</span>
                  <span><strong>1000 / 1100:</strong> ERP'den Çıkış ve Entegratöre İletim</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#fb923c' }}>
                  <span style={{ width: 20, height: 20, borderRadius: '50%', background: 'rgba(251,146,60,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.7rem', fontWeight: 700 }}>2</span>
                  <span><strong>1120 / 1163:</strong> GİB Merkez Sistemi & Şematron Doğrulama</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#22d3ee' }}>
                  <span style={{ width: 20, height: 20, borderRadius: '50%', background: 'rgba(34,211,238,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.7rem', fontWeight: 700 }}>3</span>
                  <span><strong>1200:</strong> Alıcı Posta Kutusuna Teslim</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#10b981' }}>
                  <span style={{ width: 20, height: 20, borderRadius: '50%', background: 'rgba(16,185,129,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.7rem', fontWeight: 700 }}>4</span>
                  <span><strong>1300 / 1301:</strong> Ticari Uygulama Yanıtı (Kabul / Red)</span>
                </div>
              </div>
            </div>
          </div>
