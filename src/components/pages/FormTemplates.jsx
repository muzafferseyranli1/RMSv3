  ...
  return (
    <div style={{ maxWidth: 1000 }}>
      ...
      <div className="card" style={{ padding: 20, marginBottom: 16 }}>
        ...
        {/* Form Ayarları */}
        <div style={{ marginTop: 24, borderTop: '1px dashed var(--border)', paddingTop: 20 }}>
          <h4 style={{ fontSize: '.95rem', fontWeight: 800, color: 'var(--text-strong)', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
            <i className="fa-solid fa-sliders" style={{ color: '#8b5cf6' }} /> Form Ayarları
          </h4>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
            {/* Sol Sütun: Genel Seçenekler */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {/* GPS Zorunlu */}
              <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', fontSize: '.85rem', fontWeight: 600, color: 'var(--text-strong)' }}>
                <input 
                  type="checkbox" 
                  checked={!!editing.require_geo} 
                  onChange={e => setEditing(p => ({ ...p, require_geo: e.target.checked }))} 
                  style={{ width: 16, height: 16, cursor: 'pointer', accentColor: '#8b5cf6' }}
                />
                <span>GPS Zorunlu</span>
              </label>
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', fontSize: '.85rem', fontWeight: 600, color: 'var(--text-strong)' }}>
                <input 
                  type="checkbox" 
                  checked={!!schemaJson.require_branch_selection} 
                  onChange={e => {
                    const val = e.target.checked
                    setSchemaJson(p => {
                      const nextSchema = { ...p, require_branch_selection: val }
                      if (editing.form_type === 'inspection' && !val) {
                        nextSchema.task_config = {
                          ...(p.task_config || {}),
                          enabled: false
                        }
                      }
                  ...
                  style={{ width: 16, height: 16, cursor: 'pointer', accentColor: '#8b5cf6' }}
                />
                <span>Şube Seçimi Zorunlu</span>
              </label>

              {/* Tarih ve Saati Otomatik Al */}
              <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', fontSize: '.85rem', fontWeight: 600, color: 'var(--text-strong)' }}>
                <input 
                  type="checkbox" 
                  checked={!!schemaJson.auto_date_time} 
                  onChange={e => setSchemaJson(p => ({ ...p, auto_date_time: e.target.checked }))} 
                  style={{ width: 16, height: 16, cursor: 'pointer', accentColor: '#8b5cf6' }}
                />
                <span>Tarih ve Saati Otomatik Al</span>
              </label>


              {/* Anonim Doldurmaya İzin Ver (customer_survey only) */}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', fontSize: '.85rem', fontWeight: 600, color: 'var(--text-strong)' }}>
                    <input
                      ...
                      style={{ width: 16, height: 16, cursor: 'pointer', accentColor: '#8b5cf6' }}
                    />
                    <span>Anonim Doldurmaya İzin Ver</span>
                  </label>
                  <p style={{ fontSize: '.72rem', color: 'var(--text-muted)', margin: '2px 0 0 26px', lineHeight: 1.3 }}>
                    Anonim doldurma seçildiğinde anket link/QR üzerinden herhangi bir kullanıcı girişi olmadan doldurulabilir.
                  </p>
                </div>
              )}

              {/* Bakım / Tamirat (notification_form only) */}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', fontSize: '.85rem', fontWeight: 600, color: 'var(--text-strong)' }}>
                    <input
                      ...
                      style={{ width: 16, height: 16, cursor: 'pointer', accentColor: '#8b5cf6' }}
                    />
                    <span>Bakım / Tamirat Bildirimi</span>
                  </label>
                  <p style={{ fontSize: '.72rem', color: 'var(--text-muted)', margin: '2px 0 0 26px', lineHeight: 1.3 }}>
                    Seçildiğinde bu bildirim, ekipman arıza/bakım kaydına bağlanır ve şube seçimi bypass edilir.
                  </p>
                </div>
              )}
            </div>

            {/* Sağ Sütun: Kullanım Alanı ve Görev Ayarları */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {/* Kullanım Bağlamı / Alanı */}
              <div>
                <label className="f-label" style={{ marginBottom: 8, display: 'block', fontWeight: 700 }}>Kullanım Bağlamı / Alanı</label>
              ...
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 12 }}>
                {(() => {
                  const isInspection = editing.form_type === 'inspection'
                  const isAutoTaskDisabled = isInspection && !schemaJson.require_branch_selection
                  return (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                      <label 
                        style={{ 
                          ...
                          fontSize: '.85rem', 
                          fontWeight: 700, 
                          color: isAutoTaskDisabled ? 'var(--text-muted)' : 'var(--text-strong)', 
                          cursor: isAutoTaskDisabled ? 'not-allowed' : 'pointer',
                          opacity: isAutoTaskDisabled ? 0.6 : 1
                        }}
                        title={isAutoTaskDisabled ? "Otomatik görev oluşturabilmek için Şube Seçimi Zorunlu olmalıdır." : ""}
                      >
                        <input 
                          type="checkbox"
                          checked={!isAutoTaskDisabled && !!schemaJson.task_config?.enabled}
                          disabled={isAutoTaskDisabled}
                          onChange={e => {
                            const val = e.target.checked
                            setSchemaJson(prev => {
                          ...
                          style={{ width: 18, height: 18, cursor: isAutoTaskDisabled ? 'not-allowed' : 'pointer', accentColor: '#8b5cf6' }}
                        />
                        <span>Form Gönderildiğinde Otomatik Görev Oluştur</span>
                      </label>
                      {isAutoTaskDisabled && (
                        <p style={{ fontSize: '.72rem', color: '#f59e0b', margin: '2px 0 0 26px', fontWeight: 600 }}>
                          * Otomatik görev oluşturabilmek için Şube Seçimi Zorunlu olmalıdır.
                        </p>
                      )}
                    </div>
                  )
          ...
          {schemaJson.task_config?.enabled && (
            <div style={{ marginTop: 20, borderTop: '1px solid var(--border)', paddingTop: 16 }}>
              <h3 style={{ fontSize: '0.95rem', fontWeight: 800, marginBottom: 16, color: 'var(--text-strong)', display: 'flex', alignItems: 'center', gap: 8 }}>
                <i className="fa-solid fa-bullseye" style={{ color: '#3b82f6' }} />
                Görev Kuralları ve Katılımcılar
              </h3>

              {isAnonSurvey && (
                <div style={{ padding: 12, background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 8, fontSize: '.8rem', color: 'var(--text-strong)', display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
