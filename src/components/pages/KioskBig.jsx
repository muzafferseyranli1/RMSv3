  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      const [settingsData, catRes, prodRes, chanRes, taxRes, comboRes, optionGroupsRes, devicesRes, rulesRes, assignmentsRes] = await Promise.all([
        ...
        effectiveBranchId ? db.from('kiosk_operating_hours_rules').select('*').eq('branch_id', effectiveBranchId) : Promise.resolve({ data: [] }),
        effectiveBranchId ? db.from('kiosk_terminal_operating_rules').select('terminal_id, rule_id') : Promise.resolve({ data: [] }),
      ])
    ...
    try {
      ...
      try {
        await db.from('sales').update({
          kds_status: 'pending',
          pickup_called: false,
          kiosk_service_type: selectedKioskStation?.table_service_enabled ? serviceType : 'takeaway',
