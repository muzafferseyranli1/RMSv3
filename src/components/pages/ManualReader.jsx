...
export default function ManualReader() {
  ...
  return (
    <div className={`page-enter mr-root mr-academy-active${pageDetails ? ' has-page' : ''}`}>
      <style>{`
        /* ─── LAYOUT ─── */
        .mr-root {
          display: flex;
      ...
      {/* ── Cross-Link Drawer — Beyaz Bilgi Kartı ── */}
      {drawerPageId && (
        <>
          {/* Overlay */}
          <div className="mr-link-drawer-overlay" onClick={closeDrawer} />
          ...
          <div className="mr-link-drawer">
            ...
            <div className="mr-link-drawer-body">
              {drawerLoading ? (
                <div className="mr-drawer-loading">
                  <i className="fa-solid fa-circle-notch fa-spin" style={{ fontSize: '1.8rem', color: '#6366f1' }} />
                  <span>Yükleniyor...</span>
                </div>
              ) : drawerPageDetails ? (() => {
                const dCat = categories.find(c => c.id === drawerPageDetails.category_id)
                const dCatColor = getCategoryColor(dCat?.name || '')
                const dCatIcon = getCategoryIcon(dCat?.name || '')

                // Drawer specs: spec_1..6
                const dSpecs = []
                for (let i = 1; i <= 6; i++) {
                  const lbl = drawerPageDetails.metadata?.[`spec_${i}_label`]
                  const val = drawerPageDetails.metadata?.[`spec_${i}_val`]
                  if (val) dSpecs.push({ label: lbl || `Özellik ${i}`, val })
                }
                ...
                return (
                  <>
                    ...
                    {drawerPageDetails.metadata?.product_image ? (
                      <img
                        className="mr-drawer-hero-img"
                        src={resolveImageUrl(drawerPageDetails.metadata.product_image)}
                        alt={drawerPageDetails.title}
                      />
                    ) : (
                      <div className="mr-drawer-no-img">
                        <i className={`fa-solid ${dCatIcon}`} />
                      </div>
                    )}

                    {/* Kategori bandı */}
                    {dCat && (
                      <div className="mr-drawer-cat-band" style={{ background: `${dCatColor}12` }}>
                        <div className="mr-drawer-cat-icon" style={{ background: `${dCatColor}20`, color: dCatColor }}>
                          <i className={`fa-solid ${dCatIcon}`} />
                        </div>
                        <div className="mr-drawer-cat-text" style={{ color: dCatColor }}>
                          <div className="mr-drawer-cat-label">Kategori</div>
                          <div className="mr-drawer-cat-name">{dCat.name}</div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="mr-drawer-content">
                      ...
                      {dSpecs.length > 0 && (
                        <div>
                          <div className="mr-drawer-ingredients-title">Özellikler</div>
                          <div className="mr-drawer-specs-grid">
                            {dSpecs.map((s, si) => {
                              const theme = getSpecTheme(s.label)
                              return (
                                <div key={si} className="mr-drawer-spec-card">
                                  <div className="mr-drawer-spec-label" style={{ color: theme.valColor }}>
                                    <i className={`fa-solid ${theme.icon.split(' ')[0]}`} style={{ color: theme.valColor }} />
                                    {s.label}
                                  </div>
                                  <div className="mr-drawer-spec-val" style={{ color: theme.valColor }}>{s.val}</div>
                                </div>
                              )
                            })}
                          </div>
                      ...
                      {drawerRecipeContext.length > 0 && (
                        <div>
                          <div className="mr-drawer-ingredients-title">
                            <i className="fa-solid fa-list-ul" style={{ marginRight: 4, color: dCatColor }} />
                            Bileşenler
                          </div>
                          {drawerRecipeContext.map((dr, di) => (
                            <div key={di} className="mr-drawer-ingredient-row">
                              <span className="mr-drawer-ingredient-name">{dr.name}</span>
                              <span className="mr-drawer-ingredient-qty">
                                {parseFloat(dr.qty || 0)} {dr.unit}
                              </span>
                            </div>
                          ))}
                        </div>
                ...
                <div style={{ textAlign: 'center', color: '#94a3b8', paddingTop: 48, fontSize: '0.85rem' }}>
                  <i className="fa-solid fa-circle-exclamation" style={{ fontSize: '2.5rem', display: 'block', marginBottom: 12, color: '#e2e8f0' }} />
                  Sayfa bulunamadı
                </div>
              )}
            </div>
