...
export default function Options() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: si }, { data: ca }, { data: un }, { data: ch }, { data: tx },
      ...
      db.from('sale_options').select('*').is('deleted_at',null).order('name'),
  ...
  function openEdit(item) {
    setForm({
      ...
      location: (() => { try { return JSON.parse(item.location||'[]') } catch { return [] } })(),
  ...
  // ── Save ─────────────────────────────────────────────────
  async function save() {
    if (!form.name.trim()) { setTab(0); toast('Seçenek ismi zorunludur','error'); return }
    const missingTax = (form.channel_prices||[]).filter(cp => cp.active && !cp.tax_id)
    if (missingTax.length > 0) { setTab(1); toast('Aktif kanallarda KDV seçimi zorunludur','error'); return }
    ...
    const payload = {
      ...
      same_price: form.same_price||false,
      recipe_rows: form.recipe_rows||[],
      sale_status: form.sale_status,
    }

    if (editId) {
      const { error } = await db.from('sale_options').update(payload).eq('id', editId)
      if (error) { toast('Hata: '+error.message,'error'); return }
      toast(`"${payload.name}" güncellendi`,'success')
    } else {
      const { error } = await db.from('sale_options').insert(payload)
      if (error) { toast('Hata: '+error.message,'error'); return }
  ...
  async function remove(item) {
    const { error } = await db.from('sale_options').update({deleted_at: new Date().toISOString()}).eq('id', item.id)
  ...
  async function restoreItem(item) {
    const { error } = await db.from('sale_options').update({deleted_at: null}).eq('id', item.id)
  ...
  return (
    <div className="page-enter">
      <Header
        title="Seçenekler"
        subtitle={`${filtered.length} seçenek`}
        actions={<>
          ...
          <AddButton onClick={openAdd} label="Seçenek Ekle" />
        </>}
      />
      ...
      <div className="card" style={{padding:14,marginBottom:14}}>
        <div style={{position:'relative'}}>
          ...
          <input className="f-input" placeholder="Seçenek ara…" style={{paddingLeft:30}}
      ...
      <div className="card" style={{overflow:'hidden'}}>
        {loading ? (
          ...
          <table className="tbl">
            ...
            <tbody>
              {filtered.length===0 ? (
                <tr><td colSpan={5}><div className="empty"><i className="fa-solid fa-utensils"/><p>Seçenek bulunamadı</p></div></td></tr>
              ) : filtered.map(item=>{
      ...
      {modal && (
        <div className="modal-bg open">
          <div className="modal-box" style={{width: tab===2 ? 'min(98vw, 1400px)' : 'min(96vw, 740px)', maxHeight:'92vh',display:'flex',flexDirection:'column',transition:'width .2s'}}>
            ...
            <div className="modal-body" style={{flex:1,overflowY:'auto'}}>
              ...
              {tab===0 && <div style={{display:'grid',gap:14}}>
                ...
                {/* Name */}
                <div>
                  <label className="f-label">Seçenek İsmi <span style={{color:'#ef4444'}}>*</span></label>
                  <input className="f-input" value={form.name} onChange={e=>set('name',e.target.value)} placeholder="Seçenek ismi girin"/>
                </div>
              ...
              {/* ── Tab 4: Ayarlar ── */}
              {tab===99 && <div style={{display:'grid',gap:12}}>

                {/* Ürün Ayarları başlık */}
                <div style={{background:'linear-gradient(135deg,#3b82f6,#2563eb)',borderRadius:12,
                  padding:'12px 16px',display:'flex',alignItems:'center',gap:10}}>
                  <label className="tog" onClick={e=>e.stopPropagation()}>
                    <input type="checkbox" checked={form.setting_active!==false}
                      onChange={e=>set('setting_active',e.target.checked)}/>
                    <span className="tog-sl"/>
                  </label>
                  <span style={{fontWeight:800,fontSize:'.95rem',color:'#fff'}}>Ürün Ayarları</span>
                </div>
                ...
                <div style={{marginTop:4}}>
                  <label className="f-label">İkame Ürün Seçimi</label>
                  <SearchableSelect
                    value={form.substitute_id||''}
                    onChange={nextValue=>set('substitute_id',nextValue)}
                    options={substituteOptions}
                    placeholder="Ikame urun secin"
                    searchPlaceholder="Urun ara..."
                    clearLabel="Ikame urunu temizle"
                  />
                  <p style={{fontSize:'.75rem',color:'#64748b',marginTop:6,lineHeight:1.5,
                    ...
                    Bu satış malının istatistiksel olarak yeterli veriye ulaşana kadar tahminlemelerde bu ürünün satış adetleri kullanılacaktır.
                  </p>
                </div>
              ...
              {tab===2 && (() => {
                ...
                function handleStockSelect(i, itemId) {
                  const si = stockItems.find(x=>x.id===itemId) || semiItems.find(x=>x.id===itemId)
                  const arr = [...rows]
                  arr[i] = si ? {...arr[i], stock_item_id:itemId, sku:si.sku||'', unit:si.unit||''} : {...arr[i], stock_item_id:itemId}
                  set('recipe_rows', arr)
                }
