...
function POSInner({ forcedActiveStaff = null, onStaffLogout = null, triggerPinLogin = null } = {}) {
  ...
  const groupedCart = useMemo(() => {
    ...
    return groups
  }, [isMasaMode, currentTableTicket?.cart, activeCart])
      ...
      <div key={item.id} style={{
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',
          ...
          <div style={{display:'flex',alignItems:'center',gap:4}}>
            <button className="touch-btn" onClick={() => canRemoveThisItem && setModal({ type:'item-note', itemId:item.id })}
            ...
            {!item.comboBundle && (item.portion || (item.options || []).length > 0) && (
              <button className="touch-btn" onClick={() => canRemoveThisItem && setModal({ type:'edit-item', itemId:item.id })}
          ...
          <div style={{display:'grid',gridTemplateColumns:'44px 44px 44px',
            ...
            <button className="touch-btn" onClick={() => updateQty(item.id, -1)}
              style={{background:'transparent',border:'none',borderRight:'1px solid rgba(148,163,184,.35)',
                color:!canRemoveThisItem ? '#cbd5e1' : item.qty <= 1 ? '#ef4444' : '#111827',
      ...
      {activeSpecialView !== 'orders' && (
        ...
        <div style={{flex:1,overflowY:'auto',padding:'10px',display:'flex',flexDirection:'column',gap:7}}>
          {activeCart.length === 0 ? (
            ...
            <div style={{ display:'grid', gap:8 }}>
              {activeCart.map(item => renderCartItem(item))}
            </div>
          )}
        </div>
