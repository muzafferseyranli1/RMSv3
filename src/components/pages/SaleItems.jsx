...
// ── Cloud brand picker ─────────────────────────────────────────
function CloudBrandPicker({ value, onChange, cloudBrands }) {
  const [open, setOpen] = useState(false)
  const [search, setSearch] = useState('')
  const wrapRef = useRef()
  ...
  function toggleBrand(brandName) {
    const idx = selected.indexOf(brandName)
    const next = idx > -1 ? selected.filter(b => b !== brandName) : [...selected, brandName]
    onChange(next)
  }

  function selectAll() {
    const allNames = cloudBrands.map(b => b.name)
    if (selected.length === allNames.length) {
      onChange([])
    } else {
      onChange(allNames)
    }
  ...
  const filtered = cloudBrands.filter(b => !search || b.name.toLowerCase().includes(search.toLowerCase()))
  ...
  return (
    <div ref={wrapRef} style={{ position: 'relative' }}>
      <div
        onClick={() => setOpen(o => !o)}
        style={{
          border: `1.5px solid ${open ? '#8b5cf6' : '#c4cdd9'}`,
          borderRadius: 10,
      ...
      >
        {selected.length === 0 ? (
          <span style={{ color: '#94a3b8' }}>Sanal marka seçin…</span>
        ) : selected.length === cloudBrands.length && cloudBrands.length > 0 ? (
          <span
            style={{
              display: 'inline-flex',
              alignItems: 'center',
          ...
          >
            <i className="fa-solid fa-cloud-meatball" style={{ fontSize: '.65rem' }} />
            Tüm Markalar ({selected.length})
            <span
              onClick={e => {
                e.stopPropagation()
                onChange([])
      ...
      {open && (
        ...
        >
          <div style={{ padding: '8px 10px', borderBottom: '1px solid #f1f5f9', display: 'flex', gap: 6, alignItems: 'center' }}>
            ...
            {cloudBrands.length > 0 && (
              <button
                ...
                style={{ fontSize: '.72rem', color: '#7e22ce', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 700 }}
              >
                {selected.length === cloudBrands.length ? 'Temizle' : 'Tümünü Seç'}
              </button>
            )}
          </div>
          ...
          <div style={{ maxHeight: 220, overflowY: 'auto', padding: '4px 0' }}>
            {filtered.length === 0 ? (
              <div style={{ padding: 16, textAlign: 'center', fontSize: '.82rem', color: '#94a3b8' }}>
                Sanal marka bulunamadı
              </div>
            ) : (
              filtered.map(brand => {
                const isSel = selected.includes(brand.name)
                return (
                  <div
                    ...
                    style={{
                      ...
                      borderLeft: `3px solid ${isSel ? '#8b5cf6' : 'transparent'}`,
                    }}
                  >
                    <i
                      className={`fa-solid ${isSel ? 'fa-square-check' : 'fa-square'}`}
                      style={{ color: isSel ? '#8b5cf6' : '#cbd5e1', fontSize: '.9rem', flexShrink: 0 }}
                    />
                    {brand.logo_url ? (
                      <img
                        src={brand.logo_url}
                      ...
                      <i className="fa-solid fa-cloud-meatball" style={{ color: '#8b5cf6', fontSize: '.75rem' }} />
                    )}
                    <span style={{ flex: 1, color: isSel ? '#6b21a8' : '#334155', fontWeight: isSel ? 700 : 500 }}>
                      {brand.name}
                    </span>
                  </div>
                )
...
const EMPTY = {
  ...
  location:[], cloud_brands:[], cat_id:null, acc_cat:'', acc_code:'',
  channel_prices: [],
  // Tab 2 - Seçenekler
  portions: [],
  option_groups: [],
  // Tab 5 - Reçete
  recipe_rows: [],       // [{id, ingredient_type, ingredient_id, stock_item_id, semi_item_id, sku, unit, qty, cost, waste_pct, channels, portions}]
  recipe_output_qty: 1,
  recipe_output_unit: '',
...
const TABS = [
  ...
  { label:'Satış & Fiyat',    icon:'fa-tag' },
  { label:'Seçenekler',       icon:'fa-sliders' },
  { label:'Görsel',           icon:'fa-image' },
...
export default function SaleItems() {
  ...
  const LIST_FIELDS = 'id,sku,name,short_name,deleted_at,active,auto_sku,sale_status,sale_cat_l1,sale_cat_l2,sale_cat_l3,sale_cat_l4,sale_cat_l5,cloud_brands'

  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [
        { data: si, error: siError },
        { data: ca, error: caError },
        { data: un, error: unError },
        { data: ch, error: chError },
        { data: tx, error: txError },
        { data: sk, error: skError },
        { data: smi, error: smiError },
        { data: ct, error: ctError },
        { data: bt, error: btError },
      ...
      setItems(si||[])
      const categorySnapshot = await ensureComboMenuCategory(ca || [])
      setCats(categorySnapshot.categories || sortSaleCategoriesWithComboFirst(ca || []))
      setUnits(un||[])
      setChannels(ch||[])
      setTaxes(tx||[])
      setStockItems(sk||[])
      ...
      function findSalesTax(nodes) {
        for (const n of nodes||[]) {
          if (n.type==='sirket' && n.salesTax) return n.salesTax
          const r = findSalesTax(n.children||[])
          if (r) return r
        }
        return ''
      }
      ...
      setDefaultSalesTax(findSalesTax(ct?.value||[]))
    } catch (error) {
      toast(`Satış malı ekranı yüklenemedi: ${error.message}`, 'error')
      setItems([])
    } finally {
      setLoading(false)
    }
  ...
  const assignableCats = useMemo(
    () => cats.filter(category => !isComboMenuCategory(category)),
    [cats]
  )
  ...
  const filtered = items.filter(i => {
    ...
    if (selectedBrandFilter !== 'ALL') {
      const itemBrands = Array.isArray(i.cloud_brands) ? i.cloud_brands : []
      if (!itemBrands.includes(selectedBrandFilter)) return false
    }

    const q = search.toLowerCase().trim()
    return !q || i.name?.toLowerCase().includes(q) || i.sku?.toLowerCase().includes(q) || i.short_name?.toLowerCase().includes(q)
  })
  ...
  async function openEdit(item) {
    ...
    if (error || !fullItem) {
      toast(`Satış malı açılamadı: ${error?.message || 'Kayıt bulunamadı'}`, 'error')
      return
    }

    const resolvedCategoryId = fullItem.sale_cat_l5||fullItem.sale_cat_l4||fullItem.sale_cat_l3||fullItem.sale_cat_l2||fullItem.sale_cat_l1||null
    ...
    setForm({
      ...
      cloud_brands: parseArrayValue(fullItem.cloud_brands),
      ...
      setting_active: fullItem.setting_active!==false,
    ...
    setEditId(fullItem.id); setTab(0)
    setSkuStatus(fullItem.auto_sku ? {type:'auto',msg:'Otomatik üretildi.'} : {type:'idle',msg:''})
    setModal(true)
    if (comboOnlyAssigned) {
      toast('Menuler kategorisi sadece combo menuler icin ayrildi. Bu satis mali kaydini farkli bir kategoriye tasiyip kaydetmelisiniz.', 'error')
    }
  }
  ...
  // ── Save ─────────────────────────────────────────────────
  async function save() {
    if (!form.name.trim()) { setTab(0); toast('Satış malı ismi zorunludur','error'); return }
    ...
    // Reçete: aynı malzeme + aynı porsiyon çakışma kontrolü
    const recipeRows = form.recipe_rows||[]
    const portConflicts = []
    recipeRows.forEach((row, i) => {
      const ingredientId = row.stock_item_id || row.semi_item_id || row.ingredient_id
      const ingredientType = row.ingredient_type || (row.semi_item_id ? 'semi' : 'stock')
      if (!ingredientId) return
      const rowPorts = row.portions||[]
      rowPorts.forEach(portId => {
        const clash = recipeRows.find((r,j) =>
          j !== i &&
          (r.stock_item_id || r.semi_item_id || r.ingredient_id) === ingredientId &&
          ((r.ingredient_type || (r.semi_item_id ? 'semi' : 'stock')) === ingredientType) &&
          (r.portions||[]).includes(portId)
        )
        if (clash) {
          const itemName = ingredientType === 'semi'
            ? (semiItems.find(x=>x.id===ingredientId)?.name || ingredientId)
            : (stockItems.find(x=>x.id===ingredientId)?.name || ingredientId)
          const portName = portId === '__standart__' ? 'Standart' : (form.portions||[]).find(p=>p.id===portId)?.name || portId
          portConflicts.push(`"${itemName}" → ${portName}`)
        }
      })
    })
    ...
    const selectedCategory = form.cat_id ? cats.find(category => category.id === form.cat_id) : null
    if (selectedCategory && isComboMenuCategory(selectedCategory)) {
      setTab(0)
      toast('Menuler kategorisi sadece combo menuler icin kullanilir. Normal satis mali bu kategoriye kaydedilemez.', 'error')
      return
    }
    ...
    const payload = {
      ...
      cloud_brands: JSON.stringify(form.cloud_brands || []),
      ...
      option_groups: (form.option_groups||[]).map(group => {
        ...
        const defRules = extractOptionGroupRules(def?.options)
        return {
          ...group,
          group_name: def?.name || group.group_name || '',
          min_select: Math.max(0, parseInt(defRules.min_select ?? group.min_select, 10) || 0),
          max_select: Math.max(0, parseInt(defRules.max_select ?? group.max_select, 10) || 1),
          options: defRules.options || group.options || [],
        }
      }),
    ...
    if (editId) {
      const { error } = await db.from('sale_items').update(payload).eq('id', editId)
      if (error) { toast('Hata: '+error.message,'error'); return }
      toast(`"${payload.name}" güncellendi`,'success')
    } else {
      const { error } = await db.from('sale_items').insert(payload)
      if (error) { toast('Hata: '+error.message,'error'); return }
  ...
  async function remove(item) {
    const { error } = await db.from('sale_items').update({deleted_at: new Date().toISOString()}).eq('id', item.id)
  ...
  async function restoreItem(item) {
    const { error } = await db.from('sale_items').update({deleted_at: null}).eq('id', item.id)
  ...
  const taxOptions = taxes.map(tax => ({
    ...
    selectedLabel: `${tax.name} (%${tax.rate})`,
    searchText: `${tax.name} ${tax.rate}`,
    meta: `%${tax.rate}`,
  }))
  const optionGroupOptions = optionGroupDefs.map(group => ({
    value: group.id,
    label: group.name,
    searchText: group.name,
    meta: 'Grup',
  }))
  const substituteOptions = items
    .filter(item => item.id !== editId && !item.deleted_at)
    .map(item => ({
      value: item.id,
      label: item.name,
      searchText: `${item.name} ${item.sku || ''} ${item.short_name || ''}`,
      meta: item.sku || '',
    }))
  ...
  return (
    <div className="page-enter">
      <Header
        title="Satış Malı"
        subtitle={`${filtered.length} satış malı`}
        actions={<>
          ...
          <AddButton onClick={openAdd} label="Satış Malı Ekle" />
        </>}
      />
              ...
              {filtered.length===0 ? (
                <tr><td colSpan={6}><div className="empty"><i className="fa-solid fa-utensils"/><p>Satış malı bulunamadı</p></div></td></tr>
                  ...
                  return <tr key={item.id} className={item.deleted_at?'deleted':''}>
                    <td style={{width:'130px'}}><span style={{fontFamily:'monospace',fontSize:'.8rem',fontWeight:700,color:'#475569'}}>{item.sku||'—'}</span></td>
                    <td style={{minWidth:'200px',fontWeight:600,color:'#0f172a'}}>
      ...
      >
              ...
              {tab===0 && <div style={{display:'grid',gap:14}}>
                ...
                {/* Name */}
                <div>
                  <label className="f-label">Satış Malı İsmi <span style={{color:'#ef4444'}}>*</span></label>
                ...
                <div>
                  ...
                  <CatPicker cats={assignableCats} value={form.cat_id} onChange={handleCatChange}/>
                  {comboOnlyCategory ? <p className="f-hint">{comboOnlyCategory.name} kategorisi yalnizca combo menuler icin ayrilmistir.</p> : null}
                </div>
                  ...
                  <div style={{border:'1px solid #e2e8f0',borderRadius:12,overflow:'hidden'}}>
                    ...
                    {channels.map((ch, idx) => {
                      ...
                      function updateCp(field, val) {
                        // Kanal aktifleşince default KDV otomatik gelsin
                        let updated = {...cp, [field]: val}
                        if (field === 'active' && val === true && !cp.tax_id && defaultSalesTax) {
                          updated = {...updated, tax_id: defaultSalesTax}
                        }
                      ...
                      return (
                        <div key={ch.id} style={{display:'grid',gridTemplateColumns:'1fr 100px 1fr 160px',
                          ...
                          <SearchableSelect
                            value={cp.tax_id||''}
                            onChange={nextValue=>updateCp('tax_id',nextValue)}
                            options={taxOptions}
                            placeholder="KDV seçin"
                            searchPlaceholder="Vergi ara..."
                            clearLabel="Vergiyi temizle"
                            disabled={!isActive}
                          />
                        </div>
                      )
              ...
              {/* ── Tab 2: Seçenekler ── */}
              {tab===2 && <div style={{display:'grid',gap:24}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
                    ...
                    <button className="btn-o" style={{fontSize:'.83rem'}}
                      onClick={()=>set('option_groups',[...(form.option_groups||[]),
                        {id:uid(),group_def_id:'',group_name:'',required:true,min_select:0,max_select:1,options:[]}])}>
                        ...
                        <div key={grp.id} style={{border:'1.5px solid #e2e8f0',borderRadius:12,
                          ...
                          <div style={{padding:'14px 16px',borderBottom:'1px solid #f1f5f9',
                            ...
                            <div style={{display:'grid',gridTemplateColumns:'1fr auto',gap:12,alignItems:'start'}}>
                              {/* Grup seçimi — option_groups tablosundan */}
                              <div>
                                <label className="f-label" style={{marginBottom:4}}>Seçenek Grubu</label>
                                <SearchableSelect
                                  value={grp.group_def_id||''}
                                  onChange={nextValue=>{
                                    const arr=[...(form.option_groups||[])]
                                    const def = optionGroupDefs.find(d=>d.id===nextValue)
                                    const defRules = extractOptionGroupRules(def?.options)
                                    arr[gi]={...arr[gi],
                                      group_def_id: nextValue,
                                      group_name: def?.name||'',
                                      min_select: defRules.min_select,
                                      max_select: defRules.max_select,
                                      options: defRules.options||[]
                                    }
                            ...
                            <div style={{display:'grid',gridTemplateColumns:'1fr auto',gap:12,alignItems:'end',marginTop:4}}>
                              ...
                              <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
                                <span className="badge" style={{background:'#fff7ed',color:'#c2410c'}}>
                                  Min. Seçim: {Math.max(0, parseInt(grp.min_select, 10) || 0)}
                                </span>
                                <span className="badge" style={{background:'#eff6ff',color:'#1d4ed8'}}>
                                  Max. Seçim: {Math.max(0, parseInt(grp.max_select, 10) || 1)}
                                </span>
                                <span className="badge" style={{background:'#f8fafc',color:'#475569'}}>
                                  {(grp.options||[]).length} seçenek
                                </span>
                              </div>
                          ...
                          {/* Seçenekler listesi — grup seçilince otomatik gelir */}
                          {(grp.options||[]).length>0 && (
                            <div style={{padding:'10px 16px',background:'#fafafa'}}>
                              ...
                              {(grp.options||[]).map((opt,oi)=>{
                                ...
                                return (
                                <span key={oi} style={{display:'inline-flex',alignItems:'center',gap:5,
                                  padding:'4px 10px',borderRadius:8,fontSize:'.78rem',fontWeight:600,
                                  background:'#fff',border:'1.5px solid #e2e8f0',color:'#334155'}}>
                                  <i className="fa-solid fa-circle-check" style={{color:'#22c55e',fontSize:'.65rem'}}/>
                                  {opt.name}
                                  {price>0 && <span style={{color:'#16a34a',fontWeight:700,marginLeft:2}}>
                                    +{price.toFixed(decimalPlaces)}₺
                                  </span>}
                                  {price===0 && <span style={{color:'#94a3b8',fontWeight:500,marginLeft:2,fontSize:'.7rem'}}>
                                    ücretsiz
                                  </span>}
                                </span>
                                )
              ...
              {/* ── Tab 3: Görsel ── */}
              {tab===3 && <div style={{display:'grid',gap:24}}>
              ...
              {/* ── Tab 4: Ayarlar ── */}
              {tab===4 && <div style={{display:'grid',gap:12}}>

                {/* Satır helper */}
                {[
                  { key:'sale_status',   icon:'fa-power-off',   label:'Satış Durumu',        default:true },
                  { key:'is_favorite',   icon:'fa-star',        label:'Favori Ürün',          default:false },
                  { key:'split_payment', icon:'fa-scissors',    label:'Ödemede bölünebilir:', default:false },
                  { key:'print_note',    icon:'fa-print',       label:'Açıklama Yazdırma:',   default:false },
                  { key:'hide_kitchen',  icon:'fa-eye-slash',   label:'Mutfakta Gizle',       default:false },
                ].map(row=>(
                  <div key={row.key} style={{display:'flex',alignItems:'center',justifyContent:'space-between',
                    background:'#fff',border:'1px solid #f1f5f9',borderRadius:10,padding:'12px 16px'}}>
                    <div style={{display:'flex',alignItems:'center',gap:10}}>
                      <div style={{width:32,height:32,borderRadius:8,background:'#f1f5f9',
                        display:'flex',alignItems:'center',justifyContent:'center'}}>
                        <i className={`fa-solid ${row.icon}`} style={{color:'#64748b',fontSize:'.8rem'}}/>
                      </div>
                      <span style={{fontSize:'.875rem',fontWeight:600,color:'#334155'}}>{row.label}</span>
                    </div>
                    <label className="tog">
                      <input type="checkbox"
                        checked={form[row.key] !== undefined ? form[row.key] : row.default}
                ...
                <div style={{marginTop:4}}>
                  <label className="f-label">Tahmini Verimlilik (1000 fiş)</label>
                  <div style={{display:'grid',gridTemplateColumns:'minmax(0,1fr) auto',gap:12,alignItems:'end'}}>
                    <div>
                      <input
                        ...
                        min="0"
                        step="0.1"
                        value={form.estimated_productivity || ''}
                        onChange={e=>set('estimated_productivity', e.target.value)}
                    ...
                    <div style={{
                      ...
                      {Number(form.estimated_productivity || 0).toLocaleString('tr-TR', {
                        minimumFractionDigits: 0,
                        maximumFractionDigits: 1,
                      })} / 1000 fiş
                    </div>
                  </div>
                  <p style={{fontSize:'.75rem',color:'#64748b',marginTop:6,lineHeight:1.5,
                    background:'#eff6ff',border:'1px solid #bfdbfe',borderRadius:8,padding:'8px 12px'}}>
                    <i className="fa-solid fa-circle-info" style={{color:'#3b82f6',marginRight:5}}/>
                    Sistem verimlilik hesaplayana kadar bu verimliliği kabul edecektir.
                  </p>
                </div>
              ...
              {tab===5 && (() => {
                ...
                // Porsiyon çakışma kontrolü: aynı malzeme farklı satırlarda → porsiyon farklı olmalı
                ...
                function addRow() {
                  set('recipe_rows', [...rows, {
                    id: uid(), ingredient_type:'stock', ingredient_id:'', stock_item_id:'', semi_item_id:'', sku:'', unit:'',
                ...
                function handleIngredientSelect(i, itemId) {
                ...
                // KDV hariç net fiyat (oran hesabı için)
                function chNetPrice(chId) {
                  const cp = form.channel_prices.find(x=>x.channel_id===chId)
                  const gross = parseFloat(cp?.price)||0
                  if (!gross) return 0
                  const tax = taxes.find(t=>t.id===cp?.tax_id)
                  const rate = tax ? parseFloat(tax.rate)||0 : 0
                ...
                return <div>
                  <RecipeRowsGrid
                    ...
                    onSelectItem={handleIngredientSelect}
                    getRowItemValue={row => row.stock_item_id || row.semi_item_id || row.ingredient_id || ''}
                    calcUsed={calcUsed}
                    calcCost={calcCost}
                  />
                        ...
                        <thead>
                          <tr style={{background:'#f1f5f9'}}>
                            ...
                            {activePorts.length>0 ? <>
                              <th colSpan={3} style={{padding:'6px 10px',textAlign:'center',
                                color:'#059669',fontWeight:700,borderBottom:'2px solid #e2e8f0',
                                borderLeft:'2px solid #e2e8f0',background:'#d1fae5'}}>
                                Standart
                              </th>
                              {activePorts.map(p=>(
                              ...
                              <tr key={ch.id} style={{borderBottom:'1px solid #f1f5f9',
                                ...
                                {activePorts.length>0 ? <>
                                  {(()=>{
                                    ...
                                    const netP = chNetPrice(ch.id)
                                    const ratio = netP>0?(cost/netP*100).toFixed(1):'—'
                                    const ratioNum = netP>0?cost/netP*100:0
                                    return (
                                      <>
