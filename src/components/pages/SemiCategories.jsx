...
import {
  buildSemiCategoryAccountOptions,
...
const EMPTY_FORM = {
  ...
  description: '', acc_cat: '', acc_code: '', semi_cost_account_id: '',
}
      ...
      <div style={{
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ display:'flex', gap:8, marginTop:2, flexWrap:'wrap' }}>
            ...
            {node.semi_cost_account_id && (
              <span style={{ fontSize:'.72rem', color:'#0f766e', background:'#ecfdf5', padding:'1px 7px', borderRadius:5 }}>
                Bagli yari mamul hesabi
              </span>
            )}
            {node.description && (
...
// ── Main component ───────────────────────────────────────────
export default function SemiCategories() {
  const toast = useToast()
  ...
  const accountOptions = useMemo(() => buildSemiCategoryAccountOptions(chartAccounts), [chartAccounts])
  ...
  function openEdit(cat) {
    setForm({
      ...
      semi_cost_account_id: cat.semi_cost_account_id || '',
    })
    setEditId(cat.id)
    setPreParent(null)
    setModal(true)
  ...
  async function save() {
    ...
    const payload = {
      ...
      semi_cost_account_id: form.semi_cost_account_id || null,
    }

    if (editId) {
      const { error } = await db.from('semi_categories').update(payload).eq('id', editId)
      if (error) { toast('Hata: ' + error.message, 'error'); return }
      toast(`"${payload.name}" güncellendi`, 'success')
    } else {
      const { error } = await db.from('semi_categories').insert(payload)
      if (error) { toast('Hata: ' + error.message, 'error'); return }
  ...
  async function remove(cat) {
    const { error } = await db.from('semi_categories').update({deleted_at: new Date().toISOString()}).eq('id', cat.id)
  ...
  async function restoreItem(cat) {
    const { error } = await db.from('semi_categories').update({deleted_at: null}).eq('id', cat.id)
  ...
  function handleSemiCostAccountChange(accountId) {
    const selectedAccount = accountMap.get(accountId) || null
    const legacy = resolveLegacyAccountingFields(selectedAccount, 'Yari Mamul')
    setForm(current => ({
      ...current,
      semi_cost_account_id: accountId,
  ...
  const selectedAccount = accountMap.get(form.semi_cost_account_id) || null

  return (
    <div className="page-enter">
      <Header
        title="Yarımamul Kategori Yönetimi"
        subtitle="Yarımamul hiyerarşik kategori ağacı — SKU maskı ve otomatik kod üretimi"
        actions={<>
      ...
      <CategoryHierarchyView
        ...
        sectionSubtitle="Yari mamul kategorisi baglari, SKU kurali ve alt kirilimlar"
        ...
        systemMask={sysMask}
        accountLabel="Bagli Yari Mamul Hesabi"
        accountChipLabel="Yari mamul hesabi"
        getAccountName={node => accountMap.get(node.semi_cost_account_id)?.name || ''}
      ...
      <Modal open={modal} onClose={closeModal} width={520}
        ...
        <div style={{ display:'grid', gap:14 }}>
          ...
          <div style={{ borderTop:'1px dashed #e2e8f0', paddingTop:14, display:'grid', gap:12 }}>
            <div>
              <label className="f-label"><i className="fa-solid fa-book-bookmark" style={{ color:'#6366f1', marginRight:4 }}/>Bagli Yari Mamul Hesabi</label>
              <div className="sel-wrap">
                <select className="f-input" value={form.semi_cost_account_id} onChange={e => handleSemiCostAccountChange(e.target.value)}>
              ...
              <p className="f-hint">Bu kategoriye bagli yari mamul maliyeti secilen hesaba akar.</p>
            </div>
          ...
          <div style={{ display:'none' }}>
            <div>
              <label className="f-label"><i className="fa-solid fa-calculator" style={{ color:'#6366f1', marginRight:4 }}/>Muhasebe Kategorisi</label>
              <input className="f-input" value={form.acc_cat} onChange={e => set('acc_cat', e.target.value)} placeholder="ör. Hammadde, Ticari Mal…"/>
            </div>
            <div>
              <label className="f-label"><i className="fa-solid fa-hashtag" style={{ color:'#6366f1', marginRight:4 }}/>Muhasebe Hesap Kodu</label>
              <input className="f-input" value={form.acc_code} onChange={e => set('acc_code', e.target.value)} placeholder="ör. 150, 153.01…"/>
            </div>
          </div>
