...
function getDailyReceiptPlan(branchProfile, isoDay, settings) {
  ...
  const countRandom = createRandom(`receipt-count:${isoDay}:${branchProfile.receiptCountBase}`)
  const averageRandom = createRandom(`receipt-average:${isoDay}:${branchProfile.receiptAverageBase}`)

  const weightedCount = branchProfile.receiptCountBase * weekdayFactor * randomBetween(countRandom, 0.78, 1.22)
  const receiptCount = Math.round(clamp(weightedCount, branchProfile.receiptCountMin, branchProfile.receiptCountMax))
  const weightedAverage = branchProfile.receiptAverageBase * randomBetween(averageRandom, 0.82, 1.18)
...
function buildReceiptTargets(branchProfile, receiptCount, receiptAverage, seedText) {
  const random = createRandom(`receipt-targets:${seedText}`)
...
function buildBranchDayMenuProfile(branch, isoDay, categoryPools) {
  const random = createRandom(`menu-profile:${branch.branchId}:${isoDay}`)
...
function buildReceiptLines({ branch, isoDay, receiptIndex, targetAmount, productContexts, branchProfile, menuProfile }) {
  const random = createRandom(`receipt-lines:${branch.branchId}:${isoDay}:${receiptIndex}:${branchProfile.receiptAverageBase}`)
...
function buildDiscount(lines, settings, seedText) {
  ...
  const random = createRandom(`discount:${seedText}`)
  if (random() > 0.34) {
    return { discountType: null, discountValue: 0, discountAmount: 0 }
  }
...
function buildPayments(total, branchProfile, settings, seedText) {
  const random = createRandom(`payments:${seedText}`)
